...
public class OpenLibraryService {
	...
	public Optional<Book> bookInfo(String isbn) {
		...
		Optional<OpenLibraryBook> openLibraryBook = Optional.ofNullable(response.get("ISBN:" + isbn));
	...
	public Optional<Book> bookBuildFromDto(Optional<OpenLibraryBook> olb, String isbn) {
		...
		if (olb.isPresent()) {
			...
			book.setAuthor(Optional.ofNullable(openLibraryBook.getAuthors().get(0).getName()).orElse(""));
			...
			book.setYear(Optional.ofNullable(openLibraryBook.getPublish_date()).orElse(""));
			book.setPages(Optional.ofNullable(openLibraryBook.getNumber_of_pages()).orElse(0));
